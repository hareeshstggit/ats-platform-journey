...
- 🟡 `docs/ci_schema_snapshot.sql` (2026-08-08) doesn't enable RLS on `candidates`/`candidate_documents`/`bulk_upload_jobs`/`candidate_consents` at all, and `backend-ci.yml` stamps rather than replays migrations — meaning CI's `ats_app` is unrestricted on all 4 tables and CI has zero coverage for this entire defect class (both `0057` and `0059` escaped to live use as a direct result). Minimum fix: regenerate the snapshot from the current local DB (now at `0059`, carrying the 11 candidates-domain RLS policies — 3 from `0057` + 8 from `0059`) — tracked separately since newly enabling RLS in CI may surface real failures that need their own review, not a rider on this fix. **Snapshot regenerated as a byproduct of the unrelated `0061_pos_hist_id_time_idx` migration (2026-09-01, `dev/nfr-pos-hist-index`)** — `docs/ci_schema_snapshot.sql` now reflects the DB through `0061` (the 11 candidates-domain RLS policies + `candidate_position_matches`'s own `rls_candidate_position_matches_isolation` policy, migration `0011`, genuinely never dumped before this + `0058`'s `uq_interview_level_kits_interview_id` constraint included). Still not CI-verified — no CI run has happened against the refreshed snapshot yet, so "may surface real failures" above is still open until the next CI run is watched through. **CRITICAL, caught by `principal-reviewer` before merge (2026-09-01):** the first regeneration attempt used a plain schema-only `pg_dump`, which silently dropped this file's hand-curated seed tail (bootstrap `organizations` row + 8 reference-data `COPY` blocks: `roles`/`permissions`/`role_permissions`/`lookup_values`/`currencies`/`consent_purposes`/`feature_flags`/`tenant_settings`) — this would have left every CI-seeded user with zero permissions, with `seed_dev.py`'s grant step raising no error. Fixed by re-appending the seed tail from the pre-regeneration commit and verifying non-zero row counts on all 9 reference tables (not just "loads without error", which passes on the broken file too). **Any future regeneration of this file MUST re-append this seed tail** — it is not part of a plain `pg_dump --schema-only` output.

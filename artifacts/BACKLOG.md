...
| 11 | **Tier the e2e job's 4-browser-project matrix to cut CI minutes** — user's ask 2026-08-08, after PR #221's e2e fix alone burned an est. 150-250 of GH Actions' ~2000 min/month allotment across 6 reruns. **Feasibility: YES** — Playwright supports this natively via project-level `grep`/`grepInvert` tag filtering: tag critical/NFR specs (`auth.spec.ts` — gates every downstream flow; `a11y.spec.ts` — binding WCAG mandate) `@critical`, run those across all 4 projects (chromium/webkit/mobile/reduced-motion) as today; run everything else (organizations/positions/pipeline-retry-badge — routine functional coverage) on 1-2 projects only (e.g. chromium, optionally +mobile) via `grepInvert`. **Blocking prerequisite:** the reduced matrix must run "without any errors" per the user's own requirement — the 2 flaky webkit tests already logged above (§5, retry-masked, `auth.ts`'s login helper timing out) must be genuinely fixed first, not just excluded from whichever bucket keeps webkit, or the zero-error bar is violated by construction. **Also needs, before building:** the user's own critical-vs-routine spec classification — my own guess (auth+a11y critical, rest routine) is a starting proposal, not a decision. Not built — scoping only, per user's explicit "not doing it now." | 🔴 Queued, scoped, needs user's critical-spec classification + the webkit-flake fix first |
...
- 🔴 **2 flaky `webkit`-project e2e tests, retry-masked (real CI, gh run 31250303308, PR #221 round 4, 2026-08-08)** — `organizations.spec.ts:46` (write-role create-an-organization) and `pipeline-retry-badge.spec.ts:76` (Failed/Failed-after-3-attempts). Both time out on `frontend/e2e/helpers/auth.ts:31`'s `page.waitForResponse` for `POST /auth/login` (Playwright's 30s test timeout), then pass on the automatic retry (`playwright.config.ts:16`'s `retries: 1` in CI) — the job reports success only because the retry absorbs it, not because the flakiness is gone. Suspected cause: a Next.js dev-server compile race on `webkit` specifically (the same class of first-visit-compile lag investigated earlier in this PR, this time landing on the shared login helper rather than a specific route). NOT fixed here — do not attempt a webkit-specific timing fix in this PR; needs its own investigation (e.g. whether CI's `webServer` should build+start instead of `next dev` for slow-compiling routes, per an earlier discarded hypothesis in this same PR's history).

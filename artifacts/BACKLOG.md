...
| G16 | **`audit_log` and `interview_status_history` are monthly-partitioned tables with NO automated partition-maintenance mechanism — both `docs/schema.sql` and `docs/ci_schema_snapshot.sql` only define partitions through `2026_08` (August), and zero migration, Celery beat task, or script anywhere in `backend/app/` creates future partitions.** Found live 2026-09-01 (first real CI run after the calendar rolled over to September, triggered by this session's local→origin push once Actions quota reset): `backend-ci.yml`'s `test` job failed at "Seed local test users" with `asyncpg.exceptions.CheckViolationError: no partition of relation "audit_log" found for row` — every `INSERT INTO audit_log` (and, by the same defect, every write to `interview_status_history`) fails the instant the wall clock crosses a month boundary with no partition pre-created for it. This is not a code regression from any recent PR — it is a dormant operational gap that was always going to fire on 2026-09-01 regardless of what shipped, and will fire again on 2026-10-01, 2026-11-01, etc. unless fixed. Confirmed via direct schema read (`docs/schema.sql:927-931` for `audit_log`, `:814-818` for `interview_status_history`) and a repo-wide grep for any partition-creation code outside the 2 migrations that created the *initial* June/July/August partitions — zero hits. | 🔴 | **Go-live blocker, and a live/local-dev blocker right now** — the local dev DB likely has the identical gap (same schema.sql origin) and any write to either table will start failing there too, if it hasn't already. Needs a real fix, not a one-off `ALTER TABLE ... ATTACH PARTITION`: either (a) a Celery beat task that runs monthly (or pre-creates N months ahead) and calls `CREATE TABLE ... PARTITION OF ... FOR VALUES FROM (...) TO (...)` for both tables, or (b) `pg_partman` if the project wants a battle-tested extension instead of hand-rolled scheduling. Whichever is chosen, `docs/ci_schema_snapshot.sql` needs regenerating with enough future partitions that CI doesn't hit this again before the real fix ships. Not fixed here — flagged per Gate 5 (routes through cavecrew-investigator → backend-engineer → principal-reviewer, not a main-loop fix) — needs the user's go-ahead to dispatch. |

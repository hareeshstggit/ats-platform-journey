...
**Item 5 (Gemini's path in level_kit_agent blocked the Celery worker's event loop) —
MERGED to `main`** (PR #246, commit `ef19b4a`). `fix/level-kit-gemini-async-event-loop`
branch deleted. Went with **option 1** (async-convert the existing implementation in
place), not option 2 (route through the shared `llm_gateway_providers.call_gemini`) —
reversed that lean after finding `call_gemini`'s `GenerateContentConfig` sets neither
`response_mime_type="application/json"` nor `thinking_config`, both load-bearing per
`_invoke_gemini_impl`'s own docstring (an earlier `thinking_budget=-1` attempt truncated
JSON output, silently degrading every call to `local_kit`) — option 2 would have
resurrected that exact, already-fixed bug. `principal-reviewer`: APPROVE, no findings.
Live-verified: a real Gemini call reached the network boundary, retried with correct
`asyncio.sleep` timing, degraded gracefully on exhausted retries — no hang.

**Standing lesson from this item, worth keeping**: an initial investigation pass claimed
Gemini was a dormant provider (missed `.env:50`'s `INTERVIEW_KIT_PROVIDER=gemini`
override, only checked the code default) — caught via a direct 2-command grep before
building anything on the wrong premise. Trusting an investigator's "live vs. dormant"
claim without directly checking the actual runtime config is worth a beat of suspicion
every time.

**All 4 items from the ranked top-5 BACKLOG scan (2026-09-06 evening) are now closed.**
Next: re-run a fresh BACKLOG scan/ranking pass when ready to pick the next batch, rather
than assuming anything specific is queued.
